...
import com.nd.esp.task.worker.buss.media_transcode.utils.ContentServiceUtils;
...
//        String targetFileName = NameWithoutEx+"."+extParam.get("targetFmt");
        ...
        ContentServiceUtils.downloadSync(ContentServiceUtils.getCsPathFromUrl(param.getLocation()), extParam.get("src"), param.getSession());
...
//            String targetFileName = localFilePath.substring(localFilePath.lastIndexOf(File.separator)+File.separator.length());
            ...
            if(!extParam.get("src").equals(localFilePath)) {
                ...
                if (targetFile.isFile()) {
                    ContentServiceUtils.uploadSync(targetFile, param.getTarget_location() + "/transcode/audios", param.getSession());
...
//                        "LC/MEDIA_TRANSCODE_FAIL","转码目标视频与原视频时长不符: src="+srcDuration+"s; target="+targetDuration+"s");
            ...
            String rt = null;
            if (targetFile.isFile()) {
                ContentServiceUtils.uploadSync(targetFile, param.getTarget_location()+"/transcode/videos/"+key, param.getSession());
...
//                        param.getCs_api_url(), logMsg);
        ...
        for(File thumbnail:preview.listFiles()) {
            ContentServiceUtils.uploadSync(thumbnail, param.getTarget_location()+"/transcode/previews", param.getSession());
...
//                    param.getCs_api_url(), logMsg);
        ...
        if(coverImage.exists()) {
            ...
            for(int i=1; i<=16; ++i) {
                ...
                ContentServiceUtils.uploadSync(thumbFile, param.getTarget_location()+"/transcode/previews", param.getSession());
...
//                        param.getCs_api_url(), logMsg);
            ...
            ContentServiceUtils.uploadSync(coverImage, param.getTarget_location()+"/transcode/previews", param.getSession());
...
//                    param.getCs_api_url(), logMsg);
    ...
    private static void executeTranscodeCmds(List<String> cmds, Map<String,String> extParam,
        ...
        for(String command : otherCmds) {
            ...
            if(resultValue!=0) {
                LOG.error("命令执行错误,rt="+resultValue);
    ...
    public static long getMediaMetadata(String file, String toolPath, StringBuffer logMsg,
        ...
        if(decimal.longValue() <= 0 || bVideoNoScale) {
