package nd.esp.service.lifecycle.repository.v02;

import nd.esp.service.lifecycle.repository.exception.EspStoreException;
import nd.esp.service.lifecycle.repository.index.QueryRequest;
import nd.esp.service.lifecycle.repository.index.QueryResponse;
import nd.esp.service.lifecycle.repository.model.CategoryPattern;

// TODO: Auto-generated Javadoc
/**
 * 项目名字:nd esp<br>
 * 类描述:<br>
 * 创建人:wengmd<br>
 * 创建时间:2015年2月4日<br>
 * 修改人:<br>
 * 修改时间:2015年2月4日<br>
 * 修改备注:<br>.
 *
 * @version 0.2<br>
 */
public interface CategoryPatternApi extends StoreApi<CategoryPattern>,SearchApi<CategoryPattern> {
	
	/**
	 * Gets the detail.
	 *
	 * @param patternName the pattern name
	 * @return the detail
	 * @throws EspStoreException the esp store exception
	 */
	public ReturnInfo<CategoryPattern> loadCategoryPatternByPatternName(String patternName) throws EspStoreException;
	
	
	
	/**
	 * Search.
	 *
	 * @param query the query
	 * @return the query response
	 * @throws EspStoreException the esp store exception
	 */
	public QueryResponse<CategoryPattern> search(QueryRequest query) throws EspStoreException;	
}