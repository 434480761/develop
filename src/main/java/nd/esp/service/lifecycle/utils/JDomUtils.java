...
public class JDomUtils {
    ...
    private final static Logger LOG = LoggerFactory.getLogger(JDomUtils.class);
     ...
     * @author qil
     */
    private String getRefpathHeader(String downloadPath) {
        Assert.assertNotNull("downloadPath不能为空", downloadPath);
        if(downloadPath.contains(REF_PATH_ADDON)) {
            return Constant.CS_ADDON_INSTANCE;
        } else {
    ...
    public void generatorPackPath(Map<String, Map<String, String>> pathsGrpMap,
            String path, String xmlStr, String target, boolean isInit,
            String rootPath, String header, Set<String> unzipFiles) throws Exception {
        ...
        if(isInit) {
            xmlStr = AccessXmlFile(path, header);
        }
        if(StringUtils.isEmpty(xmlStr)){//如果sdp-package.xml不存在,则直接打包根目录下的所有文件
            if(!StringUtils.isEmpty(target) && !target.equals(PACKAGE_DEFAULT_TARGET_NAME) && isInit) {
        ...
        if (destTarget == null) {
            if(isInit) {
                ...
                LOG.warn("子节点目标target节点:"+target+"获取失败");
            }
        }
        ...
        for(String src:addNodesNormal.keySet()) {
            if (src.equals(PACKAGE_TYPE_VALUE[0])) {
                ...
                if(StringUtils.isNotEmpty(dest)) {
                    ...
                    pathsGrpMap.get(subHeader).put(srcPath, "/"+src);
                } else {
        ...
        for(String src:addNodesPack.keySet()) {
            ...
            DownloadXmlThread xmlThread=new DownloadXmlThread(srcPath, subHeader, 
                    addNodesPack.get(src).getAttributeValue(PACKAGE_TARGET_NAME_ATTRIBUTE));
            tasks.add(xmlThread);
            ++index;
        }
        ...
        for(Future<DownloadXmlThread> res:results) {
            ...
            generatorPackPath(pathsGrpMap,src,thread.getStrXml(),thread.getTarget(),false,rootPath,subHeader, unzipFiles);
            ++index;
        }
    }
    ...
    class DownloadXmlThread implements Callable<DownloadXmlThread> {
        ...
        DownloadXmlThread(String src, String header, String target) {
            this.header = header;
            this.src = src;
            this.target = target;
        }
        ...
        public DownloadXmlThread call(){
            ...
            try {
                setStrXml(AccessXmlFile(src,header));
	 ...
	 * @author liuwx
	 */
	public Document buildDoc(String xmlStr) throws JDOMException, IOException {
    ...
    public String AccessXmlFile(String path,String header) throws Exception{
        if(header.equals(Constant.CS_ADDON_INSTANCE)) {
            path = Constant.CS_INSTANCE_MAP.get(Constant.CS_ADDON_INSTANCE).getPath()+path;
        }
